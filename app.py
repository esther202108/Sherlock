...
st.set_page_config(page_title="Vendor List Comparator", layout="wide")
...
def load_excel(file, key_prefix: str):
    ...
    Handles .xlsx and .xls by choosing a suitable engine.
    """
...
def normalize_name(s: pd.Series) -> pd.Series:
    """
    Normalize names to reduce mismatch due to casing/spaces.
    """
...
if file_a and file_b:
    ...
    name_col_a = st.selectbox("Name column in Excel A", list(df_a.columns), key="name_col_a")
    name_col_b = st.selectbox("Name column in Excel B", list(df_b.columns), key="name_col_b")

    vendor_col_b_choice = st.selectbox(
        "Optional vendor column in Excel B (for grouping)",
        ["(none)"] + list(df_b.columns),
        key="vendor_col_b"
    )
    vendor_col_b = None if vendor_col_b_choice == "(none)" else vendor_col_b_choice
    ...
    # ---------- Download ----------
    st.subheader("4) Download results (XLSX)")
